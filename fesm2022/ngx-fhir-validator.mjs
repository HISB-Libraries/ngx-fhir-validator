...
import { Injectable, Inject, input, output, signal, computed, ElementRef, ViewChild, Component, NgModule } from '@angular/core';
...
class NgxFhirValidatorComponent {
    constructor(fhirValidatorService, cdr) {
        ...
        // Internal state for mutable properties
        this.resultDetailsExpanded = true; // Tracks the internal state for expanding/collapsing validation results details
        ...
        this.lines = 1;
        this.width = 0;
        // Signal-based state for implementation guides
        this.igList = signal([], ...(ngDevMode ? [{ debugName: "igList" }] : []));
        this.igNameList = computed(() => {
            const names = this.igList().map(el => el.name);
            return [...new Set(names)];
        }, ...(ngDevMode ? [{ debugName: "igNameList" }] : []));
        this.igVersionList = computed(() => this.igList().map(el => el.version), ...(ngDevMode ? [{ debugName: "igVersionList" }] : []));
    ...
    ngOnInit() {
        // Initialize internal state from signal inputs
        this.resultDetailsExpanded = this.resultDetailsExpandBtnShown();
        if (this.ig()) {
